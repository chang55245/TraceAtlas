...
$TRACEHOME/build/bin/fatBinDump -i output-${1%.c}.ll -k kernel-${1%.c}.json -t kernel-${1%.c}-tik.json -o output-${1%.c}-bindump.ll

clang++-8 -shared -fuse-ld=lld-8 output-${1%.c}-bindump.ll -o output-${1%.c}.so
