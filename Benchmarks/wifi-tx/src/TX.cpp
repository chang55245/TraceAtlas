...
int main() {
  ...
  // double itime, ftime, omptime;
  // itime = omp_get_wtime();
  printf("Starting execution of the non-kernel thread [nk]\n");
   
    pthread_mutex_init(&mutex, NULL);
   
   printf("Starting execution of the non-kernel thread 11111 \n");
   std::chrono::steady_clock::time_point tn0 = std::chrono::steady_clock::now();
  ...
  //  comp_t *in_ifft;
   ...
   inbit = (unsigned char*) calloc(SYM_NUM*USR_DAT_LEN, sizeof(unsigned char));

  scram = (unsigned char*) calloc(SYM_NUM*USR_DAT_LEN, sizeof(unsigned char));
     // scram = (unsigned char**)  calloc(SYM_NUM, sizeof(unsigned char*));
      
  
   enc_out = (signed char*) calloc(SYM_NUM, sizeof(signed char*));
  ...
  //   while(frame_count < NUM_FRAMES) {
      ...
      //  std::chrono::steady_clock::time_point tn1 = std::chrono::steady_clock::now();
      // std::cout << "Time difference tn1 = " << std::chrono::duration_cast<std::chrono::nanoseconds> (tn1 - tn0).count() << "[ns]" << std::endl;
  struct timespec  start_nk, end_nk;
  ...
  //  comp_t* pilot_out;
        ...
        // pilot_out = (comp_t*) calloc(FFT_N, sizeof(comp_t));
          ...
          fft_id = 1;
              ...
              */
              ...
              // printf("malloc done %p, %d\n",ifft_input, i);

              double *ifft_output = (double*)malloc(sizeof(double)*FFT_N*2);
              for (int k = 0; k < FFT_N; k++){
                ifft_input[k*2] = (double)pilot_out[k].real;
                ifft_input[k*2+1] = (double)pilot_out[k].imag;
              }
              size_t size = FFT_N;
              bool forwardTrans = false;

              clock_gettime(CLOCK_MONOTONIC_RAW, &end_nk);
              double time_taken = (end_nk.tv_sec - start_nk.tv_sec) + (end_nk.tv_nsec - start_nk.tv_nsec) / 1000000000.0;
              printf("Time taken by non-kernel thread: %f\n", time_taken);
              std::chrono::steady_clock::time_point tfft0 = std::chrono::steady_clock::now();           
              KernelEnter("FFT");
              // printf("DASH_FFT %p, %d\n",ifft_input, i);
              DASH_FFT(ifft_input, ifft_output, size, forwardTrans);
              KernelExit("FFT");

              std::chrono::steady_clock::time_point tfft1 = std::chrono::steady_clock::now();
              // std::cout << "Time difference tfft1 = " << std::chrono::duration_cast<std::chrono::nanoseconds> (tfft1 - tfft0).count() << "[ns]" << std::endl;
              ...
              // printf("free begin %p, %d\n",ifft_input, i);
              free(ifft_input);
              // printf("free done %p, %d\n",ifft_input, i);
          ...
          //printf("[wifi-tx] Finished symbol %d\n\n", i+1);
           std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
              //  std::cout << "Time difference t2 = " << std::chrono::duration_cast<std::chrono::nanoseconds> (t2 - t1).count() << "[ns]" << std::endl;
         NonKernelSplit();
    }
      ...
      for(int i=0; i<TX_DATA_LEN - frameLength; i+=frameLength) {
  ...
  printf("[wifi]123 Execution is complete...,%f \n", time_taken);
  ...
  // printf("[WiFi-TX] Execution is complete... %f\n", omptime);
  return 0;
}
